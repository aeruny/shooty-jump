...
var acquired_double_jump = false # for beginning of game where player can't double jump
...
func play_anim(animation_name, dir) -> void:
	control = false
...
func shoot(direction):
	if control == true:
		...
		if direction == -1:
			bullet.get_child(0).set_flip_h(true) #gets sprite to flip
